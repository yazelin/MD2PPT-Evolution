...
3. **雙欄 (Two-Column)**
   ...
   - 使用 `:: right ::` 分隔。**注意：`:: right ::` 的上一行與下一行必須是空行**，否則會失效。
...
2. **背景偏好 (Background Mode)**:

   - **預設 (Mesh)**: 如果我想要「視覺衝擊」或「現代感」，請使用 `bg: mesh` 並搭配專業配色盤。

   - **簡約 (Solid)**: 如果我強調「乾淨」、「專業」或「閱讀性」，請使用純色背景 `bg: "#HEX"` (例如深灰 `#1e293b` 或米白 `#f8fafc`)，不要用 Mesh。



3. **Mesh 配色規範 (若使用 Mesh)**:

   - **嚴禁隨機亂配色！** 請從以下「專業配色盤」中選擇：
...
【風格需求】：[在此輸入風格，例如：科技藍色系、簡約深色(不要Mesh)]
