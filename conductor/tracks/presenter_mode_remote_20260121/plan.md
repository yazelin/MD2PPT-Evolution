...
- [ ] Task: 實作主控台佈局與縮圖預覽
    - [ ] 撰寫測試：驗證「當前頁」與「下一頁」縮圖邏輯是否計算正確。
    - [ ] 實作：設計主控台 UI，包含主視窗與側邊的 Next Slide 預覽。
- [ ] Task: 實作備忘錄與進度顯示系統
    - [ ] 撰寫測試：驗證備忘錄解析器能從 Markdown AST 中提取註解內容。
    - [ ] 實作：將備忘錄、計時器 (Timer) 與頁碼進度條整合至主控台介面。
- [ ] Task: 整合主控台至主應用程式
...
- [ ] Task: 打造手機遙控頁面 UI
    - [ ] 撰寫測試：驗證手機端按鈕點擊後發送正確的 P2P 指令。
    - [ ] 實作：實作 `/remote` 專用頁面，針對觸控優化的大按鈕、備忘錄顯示區。
- [ ] Task: 實作黑屏控制與全域狀態整合
    - [ ] 撰寫測試：驗證黑屏指令能穿透 P2P 通道並觸發觀眾視圖背景切換。
    - [ ] 實作：在手機端與主控台加入黑屏切換功能。
- [ ] Task: 最終回歸測試與性能優化
    - [ ] 執行所有單元測試與整合測試。
    - [ ] 優化 P2P 連線穩定性與重新連線機制。
