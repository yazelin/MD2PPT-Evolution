...
- [ ] **Task: 實作檔案與導航指令**
    ...
    - [ ] 實作動態導航指令（列出所有投影片並支援搜尋跳轉）。
- [ ] **Task: Conductor - User Manual Verification '第二階段：指令註冊' (Protocol in workflow.md)**

## 第三階段：快捷鍵與進階體驗 (Interaction)
優化鍵盤操作與最近使用功能。

- [ ] **Task: 實作全域實體快捷鍵**
    - [ ] 透過 `kbar` 的 `shortcuts` 屬性定義全域快捷鍵（如 Alt+D, Alt+P）。
- [ ] **Task: 優化 UI 反饋**
    - [ ] 實作分組顯示標題（Group Headers）。
    - [ ] 加入最近使用操作 (Recent Actions) 的優先排序。
- [ ] **Task: 編輯器焦點衝突處理**
    - [ ] 確保面板開啟時暫停 Monaco/Textarea 的預設行為。
- [ ] **Task: Conductor - User Manual Verification '第三階段：交互優化' (Protocol in workflow.md)**
